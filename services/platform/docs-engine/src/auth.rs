...
///
/// In production-like mode (`ERA_PRODUCTION` / `ERA_LICENSE_STRICT` /
/// `ERA_ENV=production` / `ERA_ENV_PRODUCTION`), returns empty if the secret is
/// unset or still the lab default — callers treat empty as 401.
pub fn jwt_secret_from_env() -> Vec<u8> {
    ...
    if production_like() {
        if raw.is_empty() || raw == DEV_JWT_SECRET {
            eprintln!(
                "era-docs-engine: ERA_IDENTITY_JWT_SECRET missing or default in production — JWT auth disabled (401)"
            );
            return Vec::new();
        }
...
/// WS: token from Authorization or `?access_token=`.
pub fn principal_from_ws(
    headers: &HeaderMap,
    query: Option<&str>,
    secret: &[u8],
) -> Result<Principal, StatusCode> {
    if let Ok(p) = principal_from_headers(headers, secret) {
        return Ok(p);
    }
    if let Some(q) = query {
        for pair in q.split('&') {
            ...
            if kv.next() == Some("access_token") {
                if let Some(tok) = kv.next() {
                    ...
                    return principal_from_headers(&h, secret);
                }
            }
        }
    }
...
mod tests {
    ...
    fn empty_secret_yields_401() {
        ...
        .unwrap();
        assert_eq!(
            principal_from_headers(&bearer(&tok), b"").unwrap_err(),
            StatusCode::UNAUTHORIZED
        );
    }

    #[test]
    fn jwt_secret_empty_in_production_when_unset() {
        clear_prod_env();
    ...
    #[test]
    fn jwt_secret_ok_in_production_when_set() {
        clear_prod_env();
        std::env::set_var("ERA_LICENSE_STRICT", "1");
        std::env::set_var("ERA_IDENTITY_JWT_SECRET", "prod-hmac-secret");
        assert_eq!(jwt_secret_from_env(), b"prod-hmac-secret");
        clear_prod_env();
    }

    #[test]
    fn jwt_secret_defaults_in_lab() {
        clear_prod_env();
        assert_eq!(jwt_secret_from_env(), DEV_JWT_SECRET.as_bytes());
        clear_prod_env();
    }
}
